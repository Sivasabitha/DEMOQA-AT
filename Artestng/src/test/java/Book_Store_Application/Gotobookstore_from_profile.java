...
public class Gotobookstore_from_profile {
    WebDriver driver;
    WebDriverWait wait;
    ...
    public void registrationLoginAndGoToBookStoreTest() {
        ...
        try {
            ...
            WebElement goToBookStoreButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//*[@id='gotoStore']")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", goToBookStoreButton);
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", goToBookStoreButton);
            System.out.println("Clicked on 'Go To Book Store' Button");

            // Added explicit wait for the Book Store header with increased timeout
            WebElement bookStorePageHeader = new WebDriverWait(driver, Duration.ofSeconds(90))
                .until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h1[contains(text(),'Book Store')]")));
            Assert.assertTrue(bookStorePageHeader.isDisplayed(), "Book Store page header is not displayed.");
